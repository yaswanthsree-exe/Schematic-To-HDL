...
import os
import tempfile
from predict_optB import (CircuitResult, find_best_model, predict_circuit,
                          find_gate_classifier, load_gate_classifier)
from ultralytics import YOLO

st.set_page_config(
    page_title="Schematic to Netlist AI [Option B]",
    page_icon="⚡",
    layout="wide",
...
st.markdown('<p class="main-title">⚡ Schematic to Netlist AI — Option B (Named Intermediate Wires)</p>',
            unsafe_allow_html=True)
st.info("**Option B test**: OCR-detected intermediate net labels (C, D …) are used as named "
        "wire signals in the annotation, netlist and equations.")
st.write("---")
...
st.sidebar.caption("predict_optB: named intermediate wires via OCR labels")
