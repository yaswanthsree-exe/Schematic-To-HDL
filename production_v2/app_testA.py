...
import os
import tempfile
from predict_optA import (CircuitResult, find_best_model, predict_circuit,
                          find_gate_classifier, load_gate_classifier)
from ultralytics import YOLO

st.set_page_config(
    page_title="Schematic to Netlist AI [Option A]",
    page_icon="⚡",
    layout="wide",
...
<style>
    ...
    .main-title {
        ...
        background: -webkit-linear-gradient(45deg, #2B8EFF, #41C6FF);
...
st.markdown('<p class="main-title">⚡ Schematic to Netlist AI — Option A (Annotation-only fix)</p>',
            unsafe_allow_html=True)
st.info("**Option A test**: Gate IDs in the annotation are replaced with OCR-detected wire labels "
        "when available. Netlist and equations are unchanged.")
st.write("---")
...
st.sidebar.caption("predict_optA: annotation display fix only")
