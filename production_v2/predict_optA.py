...
def draw_debug(
        ...
        assignment: Dict[Tuple[str,str,int], Tuple[int,str]],
        graph:      Dict[str,Dict],
        gid_ocr_output: Optional[Dict[str, str]] = None,
) -> np.ndarray:
    ...
    gid_ocr_output  — optional dict mapping gate-id → OCR output label (e.g. "G1"→"C").
                      When provided, intermediate wire labels are shown instead of raw
                      gate IDs (e.g. "AND: C, D" instead of "AND: G1, G2").
    """
    out = img.copy()
    ...
    def _disp(v: str) -> str:
        """Translate a raw gate-ID to its OCR output label when available."""
        if gid_ocr_output and v in gid_ocr_output:
            return gid_ocr_output[v]
        return str(v)

    for b in boxes:
        x, y, bw, bh = b["x"], b["y"], b["w"], b["h"]
        cv2.rectangle(out, (x,y), (x+bw,y+bh), (255,255,255), 2)
        inputs = ", ".join(_disp(v) for v in graph[b["id"]]["inputs"]) or "?"
...
def predict_circuit(
    ...
    # Option A: build gate-ID → OCR output label map for annotation display.
    # For each gate's output pin, if the net it connects to has an OCR label,
    # record it so draw_debug can show "AND: C, D" instead of "AND: G1, G2".
    gid_ocr_output: Dict[str, str] = {}
    for (gk, side, _pidx), nid in pin_nets.items():
        if side == 'out' and nid in net_names:
            gid_ocr_output[gk] = net_names[nid]

    annotated = draw_debug(img, boxes, skel_graph, assignment, gate_graph,
                           gid_ocr_output=gid_ocr_output)
