    ...
    for nid in all_nets:
        ...
        if prod is None:
            ...
            if not real_cons:
                ...
                if ocr:
                    # Option B: OCR label on an intermediate wire → use it as the
                    # named wire signal.  The producer's output gets the label so
                    # the netlist has  "assign C = G1;"  and downstream gates show
                    # "AND: C, D" in both the annotation and equations.
                    # Note: do NOT add to global_outputs — this is an internal wire.
                    if not graph[prod]["outputs"]:
                        graph[prod]["outputs"].append(ocr)
                    for (gid, idx) in real_cons:
                        pin_inputs[gid][idx] = ocr
                    log.info("Intermediate wire: net %d → '%s' (producer %s)", nid, ocr, prod)
                else:
                    for (gid, idx) in real_cons:
...
def generate_netlist(boxes: List[Dict],
    ...
    for gid in ordered:
        ...
        eq_map[gid] = eq
        # Option B: also index eq_map by any OCR-derived output names so that
        # downstream gates (which now use the OCR label as their input signal)
        # still get the fully-expanded Boolean expression.
        # e.g. eq_map["C"] = "(A & B)"  →  AND(C, D) expands to ((A & B) & (~A))
        for out_name in node["outputs"]:
            if out_name and not out_name.startswith("OUT_"):
                eq_map[out_name] = eq
