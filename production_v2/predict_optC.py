...
OPEN_KERN  = 2    # morphological opening (noise removal) — NO LONGER USED for
                  # wire cleanup (it erodes legitimate 1-px-wide wires); kept for
                  # reference / backward compat only.
CLOSE_KERN = 3    # morphological closing (gap fill)
MIN_CC_AREA = 6   # connected-component area (px) below which a blob is treated
                  # as speckle noise and removed.
THIN_WIRE_MIN_AREA = 80  # min area (px) of an opening-removed pixel group for it
                  # to be rescued as a real thin wire.  A genuine deleted wire is
                  # a long line (large area); noise specks and 1-px edge shavings
                  # of thick wires are smaller and stay removed.
# Toggles for the two new fixes.  Set False to reproduce pre-fix optC behaviour
# — used by the regression harness to isolate each fix's impact.
ENABLE_FIX_A = True   # thin-wire rescue in preprocess
ENABLE_FIX_B = True   # input-pin re-snap to nearest net
...
def preprocess(img: np.ndarray, boxes: List[Dict]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # Estimate the background colour from a 5-px border strip.
    # Using background fill (instead of hardcoded 255) is critical for coloured-bg
    # schematics: filling cyan-bg images with white creates artificial blobs that
    # confuse binarisation.  For white-bg schematics the fill stays white.
    h_img, w_img = clean.shape[:2]
    bw = min(5, h_img // 4, w_img // 4)   # border width (never >25% of image)
    border_pix = np.concatenate([
        clean[:bw,  :   ].reshape(-1, 3),
        clean[-bw:, :   ].reshape(-1, 3),
        clean[:,  :bw   ].reshape(-1, 3),
        clean[:, -bw:   ].reshape(-1, 3),
    ])
    bg_fill = np.median(border_pix, axis=0).astype(np.uint8)
    log.info("preprocess: bg_fill BGR=(%d,%d,%d)", *bg_fill)
    ...
    if ENABLE_FIX_A:
        # Surgical thin-wire rescue.  A 2x2 MORPH_OPEN erodes a 1-px-wide wire
        # to nothing — e.g. a thin branch that taps a bus and drops straight
        # down into a downstream gate input.  Deleting that wire silently
        # removes a real electrical connection (the branch input is then
        # mis-detected as a separate/auto-named net).
        #
        # The deleted wire is usually only a SEGMENT of a larger component that
        # otherwise survives the opening, so restoring whole wiped components is
        # not enough.  We look at the pixels the opening actually removed and
        # group them.  Two kinds of pixels get removed by a 2x2 opening:
        #   (1) a genuine thin wire that was eroded away ENTIRELY — leaves a real
        #       gap (no surviving wire nearby), and
        #   (2) a 1-px "shaving" off the edge of a thick wire whose core survives
        #       1 px away.
        # Restoring (2) thickens wires everywhere and perturbs the skeleton on
        # nearly every schematic, so we restore ONLY groups that are both large
        # enough to be a real wire AND isolated from any surviving wire (i.e. a
        # true gap).  This keeps the change invisible to circuits that have no
        # deleted thin wire.
        removed = cv2.subtract(binary, opened)
        opened_dil = cv2.dilate(
            opened, cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3)))
        n_lbl, lbls, stats, _ = cv2.connectedComponentsWithStats(removed, connectivity=8)
        for li in range(1, n_lbl):
            if stats[li, cv2.CC_STAT_AREA] < THIN_WIRE_MIN_AREA:
                continue                       # speckle / short shaving
            comp = (lbls == li)
            # Fraction of this removed group that sits right next to a surviving
            # wire pixel.  High → edge shaving of a thick wire (skip); low → the
            # group is a genuine deleted wire that left a gap (restore).
            adj_frac = float((opened_dil[comp] > 0).mean())
            if adj_frac < 0.30:
                opened[comp] = 255
    binary = opened
...
def detect_junction_dots(
    ...
    # ── Pass 1: DT thickness ──────────────────────────────────────────────────
    # 2.0× threshold (was 2.5×) — more sensitive without causing false positives
    # at wire crossings (crossings reach ~1.5–1.8× wire_r under the DT).
    thresh   = max(2.5, wire_r * 2.0)
    thick    = (dt >= thresh).astype(np.uint8)

    # Dynamic area bounds:
    #   dot radius ≈ 1.5–4× wire_r  →  area ≈ π × r²
    min_area = max(6,   int(np.pi * (1.5 * wire_r) ** 2 * 0.5))
    max_area = min(600, int(np.pi * (4.5 * wire_r) ** 2 * 1.2))

    n, _lbl, stats, cents = cv2.connectedComponentsWithStats(thick)
    ...
    for i in range(1, n):
        ...
        fill = area / max(1, bw * bh)          # circle ≈ 0.78
        if min_area <= area <= max_area and ar < 2.5 and fill >= 0.50:
            centers.add((int(cents[i][0]), int(cents[i][1])))
...
def gate_pin_centers(b: Dict) -> Dict[str, List[Tuple[int,int]]]:
    ...
    return {
        "in":  [(gx + PIN_MARGIN,      int(gy + gh * f)) for f in cfg["in"]],
...
def _resnap_input_pins_to_nearest_net(
        boxes:      List[Dict],
        assignment: Dict[Tuple[str,str,int], Tuple[int,str]],
        ...
        edge_net:   List[int],
        shape:      Tuple[int, int],
        near_r:     int = SNAP_R_TIGHT,
) -> Dict[Tuple[str,str,int], int]:
    """Re-snap gate INPUT pins to the physically-nearest net.

    Root problem this fixes: when a wire terminates in a thick-wire *junction
    mesh* right at a gate pin (common where a branch turns 90° into a gate),
    the wire has no skeleton ENDPOINT near the pin.  Endpoint-based pin
    assignment then snaps the pin to whatever endpoint IS nearby — often a
    residual gate-body skeleton fragment that survived gate erasure (e.g. an
    inverter-triangle edge).  The pin then reports the wrong net.

    Here we rebuild a per-pixel net-label image and, for each input pin, find
    the nearest labelled wire pixel.  If that nearest net is meaningfully
    closer than the currently-assigned wire endpoint, and is not already
    consumed by another input pin of the same gate, we override the pin's net.
    For correctly-assigned pins the nearest net pixel already belongs to the
    assigned net, so nothing changes — making this a low-risk correction.
    """
    h, w = shape[:2]
    net_lbl = np.zeros((h, w), dtype=np.int32)
    ...
    for b in boxes:
        ...
        for pidx, (px, py) in enumerate(centers["in"][:n_in]):
            pkey    = (gid, "in", pidx)
            cur_net = pin_nets.get(pkey)

            # Distance from the pin to the wire-endpoint it is currently bound to.
            cur_d = 1e9
            if pkey in assignment:
                eid_c, end_c = assignment[pkey]
                path = skel_graph.edges[eid_c].path
                ex, ey = path[0] if end_c == "a" else path[-1]
                cur_d = ((ex - px) ** 2 + (ey - py) ** 2) ** 0.5

            # Nearest labelled net pixel within near_r of the pin.
            x0 = max(0, px - near_r); x1 = min(w, px + near_r + 1)
            y0 = max(0, py - near_r); y1 = min(h, py + near_r + 1)
            sub = net_lbl[y0:y1, x0:x1]
            ys, xs = np.where(sub > 0)
            if len(xs) == 0:
                continue
            d2 = (xs + x0 - px) ** 2 + (ys + y0 - py) ** 2
            k  = int(np.argmin(d2))
            near_net = int(sub[int(ys[k]), int(xs[k])]) - 1
            near_d   = float(d2[k]) ** 0.5
            if near_net == cur_net:
                continue

            # Don't collapse two inputs of the same gate onto one net.
            others = {pin_nets.get((gid, "in", q))
                      for q in range(n_in) if q != pidx}
            if near_net in others:
                continue

            # Only override when the nearest wire is at the pin AND clearly
            # closer than the currently-bound (residual) endpoint.
            if near_d <= near_r and near_d + 8.0 < cur_d:
                log.info("Pin re-snap: %s net %s -> %s (near_d=%.1f cur_d=%.1f)",
                         pkey, cur_net, near_net, near_d, cur_d)
                pin_nets[pkey] = near_net
    return pin_nets
...
def build_nets(
    ...
    # Union segments sharing a junction node — crossing-aware
    for nidx, e_list in node_edgs.items():
        if skel_graph.node_type[nidx] != "junction":
            continue
        nx, ny = skel_graph.node_xy[nidx]
        # A degree-4+ node where EVERY edge is ≤ MIN_EDGE_LEN long is part of
        # a "thick-wire mesh": the 2-pixel-wide wire skeleton creates a dense
        # grid of 1-1.4px cross-edges between adjacent pixels.  These are NOT
        # real wire crossings — all edges belong to the SAME physical wire.
        # Merge them all instead of running crossing detection, which would
        # otherwise mis-identify diagonal cross-edges as "straight-through"
        # crossing pairs and leave the main wire chain disconnected.
        #
        # Also handle MIXED junctions: a long wire (bus) meeting a thick-wire
        # mesh horizontal branch creates a degree-4+ node with some long and some
        # short edges.  The short edges are mesh-contact spurs of the same wire;
        # they must be merged with the long edge instead of being treated as a
        # crossing.  Detect this via has_mesh: ≥2 short edges at the node means
        # we are at a thick-wire mesh contact point, not a genuine wire crossing.
        n_short = sum(1 for eid in e_list
                      if _edge_euclidean_len(skel_graph.edges[eid]) <= MIN_EDGE_LEN)
        all_short = len(e_list) > 3 and n_short == len(e_list)
        has_mesh  = n_short >= 2          # mesh contact alongside real wire(s)
        if len(e_list) <= 3 or _near_dot(nx, ny) or all_short or has_mesh:
            # T-junction, solder dot, or thick-wire mesh: all edges same net
            for i in range(1, len(e_list)):
                uf.union(e_list[0], e_list[i])
        else:
            # Degree-4+ node without a dot and with at least one real-length
            # edge: likely a wire crossing.  Only merge straight-through pairs.
            _merge_crossing_pairs(e_list, skel_graph, nidx, uf)
            ...
            for k in order:
                ...
                if not ok2:
                    ...
                    for t in range(stps2 + 1):
                        ...
                        if not found3:
                            ok2 = False; break
                # Fallback 3: local connected-component check.
                # Handles T-junction corners where the wire path between the
                # skeleton endpoint and the branch point bends 90°.  The three
                # straight-line / dilation checks above all fail at the bend
                # because they walk diagonally across the corner, missing the
                # actual wire pixels.  A small-box CC check finds the connection
                # via the real (bent) wire path.
                # Safe vs. crossings: ep_ep_bridged_nodes guards already-connected
                # endpoints; and crossing wires are far apart in endpoint space.
                if not ok2:
                    box_r  = MAX_SKEL_GAP + 2
                    bx0_cc = max(0, min(xi, px2) - box_r)
                    bx1_cc = min(bw, max(xi, px2) + box_r + 1)
                    by0_cc = max(0, min(yi, py2) - box_r)
                    by1_cc = min(bh, max(yi, py2) + box_r + 1)
                    roi_cc = binary[by0_cc:by1_cc, bx0_cc:bx1_cc]
                    _, cc_roi = cv2.connectedComponents(roi_cc.astype(np.uint8),
                                                        connectivity=8)
                    sy_cc = yi - by0_cc; sx_cc = xi - bx0_cc
                    ty_cc = py2 - by0_cc; tx_cc = px2 - bx0_cc
                    if (0 <= sy_cc < cc_roi.shape[0] and
                            0 <= sx_cc < cc_roi.shape[1] and
                            0 <= ty_cc < cc_roi.shape[0] and
                            0 <= tx_cc < cc_roi.shape[1]):
                        cc_src = int(cc_roi[sy_cc, sx_cc])
                        cc_tgt = int(cc_roi[ty_cc, tx_cc])
                        if cc_src > 0 and cc_src == cc_tgt:
                            ok2 = True
                            log.debug("CC-bend bridge: ep(%d,%d)→interior(%d,%d) "
                                      "eid=%d←eid=%d", xi, yi, px2, py2,
                                      ei_list[0], eid_j)
                if ok2:
...
def ocr_net_names(
    ...
    def _snap(cx: float, cy: float) -> Optional[int]:
        """Return net id of nearest wire endpoint within OCR_SNAP_R, or None."""
    ...
    for (bbox, text, conf) in ocr_res:
        ...
        if nid is not None:
            net_names[nid] = text
            log.info("OCR net name: net %d → '%s'", nid, text)
            ...
            if isinstance(_existing, str) and _existing in global_inputs:
                ...
                if _in_nid_wp is not None:
                    ...
                    if _in_nid_wp not in demoted_nets:
                        if _net_path_px(_in_nid_wp) >= MIN_PRIMARY_PIX:
...
def predict_circuit(
    ...
    # ── Stage 7a.6: Re-snap input pins that grabbed a gate-body residual ──────
    # When a wire ends in a thick-wire mesh right at a gate pin it has no
    # skeleton endpoint there, so the pin may have been bound to a residual
    # gate-body skeleton fragment (wrong net).  Re-bind such pins to the net
    # whose wire pixels are physically closest to the pin.
    if ENABLE_FIX_B:
        pin_nets = _resnap_input_pins_to_nearest_net(
            boxes, assignment, pin_nets, skel_graph, edge_net, binary.shape)
