...
import os
import tempfile
from predict_optC import (CircuitResult, find_best_model, predict_circuit,
                          find_gate_classifier, load_gate_classifier)
from ultralytics import YOLO

st.set_page_config(
    page_title="Schematic to Netlist AI [Option C]",
    page_icon="⚡",
    layout="wide",
...
<style>
    ...
    .main-title {
        ...
        background: -webkit-linear-gradient(45deg, #2BFF6E, #00C9FF);
...
st.markdown('<p class="main-title">⚡ Schematic to Netlist AI — Option C (T-junction fix)</p>',
            unsafe_allow_html=True)
st.info(
    "**Option C fix**: Adds a local connected-component fallback to the endpoint-to-interior "
    "bridge pass. This catches T-junction corners where the branch wire meets the spine at 90° "
    "— the case that was causing B's branch to be treated as a separate input (D) instead of B."
)
st.write("---")
...
st.sidebar.caption("predict_optC: CC-bend T-junction bridge fallback")
